...
int main() {
    ...
    const std::string user_defined_type = "MyType";

    const auto program = new Token_Protocol{ 0, 0, PROGRAM };
    const auto const_ = new Token_Keyword{ 0, 0, KEY_DATATYPE_CONSTANT };
    const auto unsigned_ = new Token_Keyword{ 0, 5, KEY_DATATYPE_UNSIGNED };
    const auto int_ = new Token_Keyword{ 0, 5, KEY_DATATYPE_INTEGER };
    ...
    const auto val = new Token_Number{ 0, 10, "0x34" };
    ...
    Token_List::user_defined_type.insert( user_defined_type );

    try {
        Qurio_Parser::scan_type( node_const, RETURN_TYPE );
    ...
    for( const auto it : * node_prog ) PRINT_DEBUG_IT( *it );
